...
def count_trees(arr, right, down):
    ...
    return np.sum([arr[down*i, right*i % width] for i in range((height-1)//down+1)])
...
print(np.prod([count_trees(arr, right_list[i], down_list[i]) for i in range(len(right_list))]))
